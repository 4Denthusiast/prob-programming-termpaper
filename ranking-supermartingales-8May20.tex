...
\begin{definition}\rm
A sequence of random variables $(Y_n)_{n \geq 0}$ adapted to a filtration $(\calF_n)_{n \geq 0}$ is a \emph{supermartingale} if for all $n \in \omega$, $\expect{|Y_n|} < \infty$, and $\expect{Y_{n+1} \mid \calF_n} \leq Y_n$.
...
Let $S$ and $T$ be stopping times adapted to $(\calF_n)_{n \geq 0}$.
...
Let $(X_n)_{n \geq 0}$ be a supermartingale, and $(T_n)_{n \geq 0}$ a sequence of increasing stopping times, then $(X_{T_n})_{n \geq 0}$ is a supermartingale adapted to $(\calF_{T_n})_{n \geq 0}$ if one of the following conditions holds:
\begin{enumerate}
...
\label{thm:rank-PAST}
Let $(Y_n)_{n \geq 0}$ be a $\epsilon$-ranking supermartingale, and set $T(s) := \min \set{n \mid Y_n(s) = 0}$. 
...
The $\calF_n$-measurability of $M_n$ (and hence $\#\mathrm{draw}_n$) follow from \citep{DBLP:conf/icfp/BorgstromLGS16};
...
\bblambda s . f(E_i[\underline{\sigma_i(s)}][\underline{\rho(s)}])[s \in U_i]
\label{eqn:f 2 n+1}
\end{equation}
where 
\begin{itemize}
...
\label{eqn:truncate}
\end{equation}
where $A_{\leq m} := \set{s_{\leq m} \mid s \in A}$, and $\Leb_n$ is the Lebesgue measure on $I^n$.
...
Observe that $T_0, T_1, T_2, \cdots$ are an increasing sequence of stopping times adapted to $(\calF_n)_{n \geq 0}$, and each $T_i$ is bounded.
As $(X_n)_{n \geq 0}$ is a supermartingale (\Cref{thm:rankable gives supermartingale}), it follows from \Cref{thm:optional sampling} that $(Y_n)_{n \geq 0}$ is a supermartingale (in fact, a $1$-ranking one).
