...
\lo{Is (1) too general for the conjecture?
Claim (1) assumes an arbitrary a.s.~finite stopping time $T$. This seems too general for the conjecture, which assumes a ``strongly'' AST $M \in \Lambda^0$.
Regardless of the validity of (1), in order to prove the conjecture, I think we would need a more specific version of (1) that assumes a stopping time about terminating runs of $M$ --- such as the stopping time of definition (\ref{eq:y stopping time}) which only counts $\tY$-reduction steps --- so that the antitone-strict supermartingale that is constructed has the necessary structure to yield a ranking function for $M$.}
...
We could define $T_1$ as the least number satisfying $\mathbb P[T < T_1] \geq \frac{1}{4}$.} \akr{Sorry, I just wrote it wrong. I assume this ($\mathbb P[T > T_n]$ instead of $\mathbb P[T < T_n]$) resolves your concern.}
We will then define a supermartingale $(Y_n)_{n \in \omega}$ and nonrandom sequence $(Y'_n)_{n \in \omega}$ such that $Y_n = 0$ iff $T < n$ \changed[lo]{a.s.} \akr{I see no reason that this can't just be true everywhere, as this is part of the definition of $Y_n$. Likewise for the other ``a.s.'' you added.}, $Y_n = Y'_n$ iff $T \geq n$ \changed[lo]{a.s.}, and $Y'_n \geq 2^k$ for $n \geq T_k$.
