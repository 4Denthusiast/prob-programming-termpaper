...
$y_n$ is then defined recursively by
\begin{align*}
    y_0 = 2, \quad
...
Although the stronger induction hypothesis was necessary for the induction to work, the only reason this is needed is to prove that $y_n > 0$ for all n, which implies that $Y_n \geq 0$. The other condition for $(Y_n)$ to be an antitone-strict supermartingale is
\begin{align*}
    & \expect{Y_{n+1} \mid \mathcal F_n} \\
    = & \expect{y_{n+1} 1_{T \geq n+1} \mid \mathcal F_n} \\
    = & y_{n+1} \expect{1_{T \geq n+1} \mid \mathcal F_n} \\ 
    = & y_{n+1} 1_{T \geq n} \frac{\mathbb P[T > n+1]}{\mathbb P[T > n]} \\
    = & (y_n - \epsilon(y_n)) 1_{T \geq n} \\
    = & Y_n - \epsilon(y_n) 1_{T \geq n}
\end{align*}
