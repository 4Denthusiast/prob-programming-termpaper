...
\begin{definition}\rm
\begin{enumerate}
\item A sequence of random variables $(Y_n)_{n \geq 0}$ adapted to a filtration $(\calF_n)_{n \geq 0}$ is a \emph{supermartingale} if for all $n \in \omega$, $Y_n$ is integrable (i.e.~$\expect{|Y_n|} < \infty$), and $\expect{Y_{n+1} \mid \calF_n} \leq Y_n$ a.s.~\changed[lo]{(i.e.~$\forall A \in \calF_n$, $\int_A \dif \mu \, Y_{n+1} \leq \int_A \dif \mu \,Y_n$).}
...
For 1, observe that $T_0, T_1, T_2, \cdots$ are an increasing sequence of stopping times, each is bounded \lo{Prove it.} and adapted to $(\calF_n)_{n \geq 0}$.
Then, since $(f(M_n))_{n \geq 0}$ is a supermartingale also adapted to $(\calF_n)_{n \geq 0}$ (\Cref{thm:rankable and strict rankable}),
