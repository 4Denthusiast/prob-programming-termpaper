...
Intuitively $Y_n$ gives the rank of the program after $n$ steps of computation.
(Assume that the rank of a term is 0 iff it is a value.) In a $\epsilon$-ranking supermartingale, each computation step causes a strict decrease in rank of at least $\epsilon$, provided the term in question is not a value.
...
\label{thm:rank-PAST}
Let $(Y_n)_{n \in \omega}$ be a $\epsilon$-ranking supermartingale, and define the r.v.~$T$ by $T(s) := \min \set{n \mid Y_n(s) = 0}$. 
...
X_n(s) & := f(M_n(s)) \nonumber \\
Y_n(s) & := X_{T_n}(s) \nonumber \\
T(s) &:= \min \set{n \mid Y_n(s) = 0} \label{eq:y stopping time}
\end{align}
