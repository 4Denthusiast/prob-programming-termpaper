...
\begin{remark}
\changed[lo]{The original definition of ranking supermartingale, due to \citep{DBLP:conf/cav/ChakarovS13,DBLP:conf/popl/FioritiH15}, does not mention stopping time explicitly:
...
\lo{\Cref{lem:rank-PAST} is a slight extension of \citep[Lemma 5.5]{DBLP:conf/popl/FioritiH15}. In op.~cit., the stopping time $T$ is assumed to be the random variable $s \mapsto \min\set{n \mid Y_n(s) = 0}$.}
