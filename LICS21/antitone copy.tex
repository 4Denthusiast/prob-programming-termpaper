...
\begin{definition}
\changed[akr]{The definition of \emph{antitone ranking function} $f$ for $M \in \Lambda^0$ is the same as that of ranking function except that in the case of $\tY$-redex, 
...
\changed[lo]{Note that antitone ranking functions are actually a generalisation of ranking functions, even though the way we reference them may suggest that they are a type of ranking functions.}
...
\lo{I believe the ranking function method can be used to prove soundness of the Beutner-Ong counting-based method (which reduces the AST problem of first-order (possibly non-affine) recursive PPCF programs to random walk on $\mathbb{N}$), based on Theorem 5.9 and Lemma 5.10 in \emph{op.~cit.}}
