...
\subsection*{Example \ref{ex:raven complex} (Continuous random walk)}
...
\subsection*{Example \ref{ex:Fair-in-the-limit random walk} (Fair-in-the-limit random walk) \citep[\S 5.3]{DBLP:journals/pacmpl/McIverMKK18}}
\[
\underbrace{\big
(\tY \, \lambda f x .\ 
...
\subsection*{Example \ref{ex:non-affine recursion easy} (Non-affine recursion)}
Let 
\[
...
\subsection*{Example \ref{ex:non-affine continuous} (More complex non-affine recursion)}
Let
\begin{align*}
...
\subsection*{Example \ref{ex:higher-order recursion} (Higher-order recursion)}
