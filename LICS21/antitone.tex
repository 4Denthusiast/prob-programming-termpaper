...
\dfrac{
\mathbb P[T > n] \, \expect{Y_n \mid T > n} + \mathbb P[T \leq n] \, \expect{Y_n \mid T \leq n} - \mathbb P[T \leq n] \, \expect{Y_n \mid T \leq n}
}{\mathbb P[T > n]}
...
\end{proof}
\begin{example}
To demonstrate why it is necessary that $\epsilon(x) > 0$ for all $x$, not merely for those values the ranking function actually takes, take the probability space $(\Omega, \calF, \mathbb{P})$ where $\Omega$ is the closed interval of reals $[-1, 1]$, $\calF$ the Borel $\sigma$-algebra, and $\mathbb{P}$ the corresponding Lebesgue probability measure.
...
\subsection{Generality and incompleteness}
...
The number of functions $\mathbb N \to \mathbb N$ is equal to $|\mathbb R|$, therefore by assigning some code and uncurrying, it is possible to define a function $Z : \mathbb R \times \mathbb N \to \mathbb N$ such that $Z(r, \cdot)$ can be any function $\mathbb N \to \mathbb N$ given the right value of $r \in [0,1]$, and it is even possible to do this in such a way that $Z$ is measurable. As PPCF contains every measurable real function as a primitive, it is therefore possible for a PPCF program to randomly pick a function $\mathbb N \to \mathbb N$ so that every such function is possible.
...
However for any candidate $t$, there is some $r$ such that $Z(r, \cdot)$ is much faster growing than $n \mapsto t^{-1}(\frac 1 n)$, therefore the condition for Theorem \ref{thm:towards completeness} is not satisfied for $M$. 

It turns out that $M$ is not antitone rankable at all. 
Every antitone $\epsilon : \nnReal \to \pReal$ defines some correspondence between expected $\tY$ runtimes of terms and how high the ranking function value must be,
therefore there is some $f: \mathbb N \to \mathbb N$ sufficiently fast growing that the ranking function value required for each 
\[
\Big(
\lambda m. 
\big(
...
The unusual function $Z$ also plays a crucial role in the construction, 
