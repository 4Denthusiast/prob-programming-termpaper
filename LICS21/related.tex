...
The \Cref{def:more general red} of redexes and which positions are acceptable to reduce at is sufficiently restrictive to guarantee confluence, but there are several cases where it is a more restrictive than is necessary for this purpose. For example, if the argument of a function is not yet a value, but its reduction to a value would be deterministic, applying the function before reducing its argument would not cause any problematic duplication of random samples. A more complete characterisation of which redexes can be reduced without breaking confluence could be interesting.
