...
\lo{There are some niggling (but inconsequential) issues about $\tscore$ that reviewers may pick on. Since $\tscore$ does not really affect termination, we don't lose much at all by restricting ourselves to Probabilistic PCF (i.e.~by omitting $\tscore$ altogether). What do you think? (We could add a remark to say our analysis extends straightforwardly to $\tscore$.)}
