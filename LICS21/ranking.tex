...
Despite its simplicity, its $Rch$ contains all the terms in \Cref{fig:geometric distribution}.
\lo{\Cref{fig:geometric distribution} is confusing: the arrow means either one-step reduction, or its reflexive transitive closure, or ``has the same syntactic shape''. I think \Cref{fig:geometric distribution 1} is not confusing (arrow does mean one-step reduction; note $r'$), and it would be correct to say that $\mathit{Rch}(\tY \, \Theta \, \underline 0)$ contains all the terms in \Cref{fig:geometric distribution 1}, for each $i \geq 0$.}
\begin{figure}
...
\centering
\begin{tikzpicture}[node distance=0.5em, anchor=west]
    \node (1) at (0,-0) {$\tY\, \Theta\, \underline i$};
    \node (2) at (0,-1) {$(\lambda z. (\lambda n. \tif{\tsample \underline{- 0.5} < 0}{n}{\tY\, \Theta\, (n \underline{+ 1})}) z)\, \underline i$};
    \node (3) at (0,-2) {$(\lambda n. \tif{\tsample \underline{- 0.5} < 0}{n}{\tY\, \Theta\, (n \underline{+ 1})})\, \underline i$};
    \node (4) at (0,-3) {$\tif{\tsample \underline{- 0.5} < 0}{\underline i}{\tY\, \Theta\, (\underline i\, \underline{+ 1})}$};
    \node (5) at (0,-4) {$\tif{\underline r\, \underline{- 0.5} < 0}{\underline i}{\tY\, \Theta\, (\underline i\, \underline{+ 1})}$};
