...
\end{align*}
In each of these cases, $M \mid \alpha$ is the \defn{redex}, and the \defn{reduction takes place at $\alpha$}.
\end{definition}
...
The reflexive transitive closure $\sim^*$ of this relation is used to define the set of \defn{potential positions} $L(M) = L_0(M) / \sim^*$, and each equivalence class can be considered as the same position as it may occur across multiple reachable skeletons. 
...
\begin{definition}
An \defn{antitone ranking function on $M$ with respect to a reduction strategy $r$} is a measurable function $f:\mathit{Rch}_r(M) \to \mathbb{R}$ such that $f(N) \geq 0$ for all $N$, and there exists an antitone function $\epsilon : \nnReal \to \pReal$ such that
\begin{itemize}
...
\end{itemize}
Any closed term for which an antitone ranking function with respect to a reduction strategy $r$ exists is called \defn{antitone rankable} with respect to $r$. 
\end{definition}

\begin{definition}
An \defn{antitone sparse ranking function on $M$ with respect to a reduction strategy $r$} is a partial function $f : \mathit{Rch}_r(M) \rightharpoonup \mathbb R$ such that there exists some antitone function $\epsilon : \nnReal \to \pReal$ such that
\begin{inparaenum}[(i)]
...
If the antitone function is just the constant function $1$ instead, then it is called a \defn{(sparse) ranking function on $M$ with respect to $r$} instead, and the term is called \defn{rankable} with respect to $r$.
