...
\changed[lo]{We are thus led to consider the reduction relation on skeletons (and positions in a skeleton), which can be extended from the definitions on terms in the obvious way,} 
...
\[\begin{array}{l}
\bigcup_{r \in [0,1]} \Big\{s \in I^{L_s(A[\tsample])} \mid \\
\qquad\qquad\qquad\quad s([A[\tsample]], \mathsf{if}_1;\underline{-}_1;@_1;\mathsf{if}_1) = r, \\
\qquad\qquad\qquad\quad s([A[\tsample], A[\underline r], \ldots, B, \mathsf{if}_1) > 0.5 \Big\}.
\end{array}\] 
\lo{There are a number of spurious square bracket in the predicate of the set comprehension above.}

\lo{We have just defined $L_0(M)$, but what is $L_s(\cdot)$ (subscript $s$)? I assume it is the version of $L_0(M)$ that labels samples by terms.} 
...
In the reduction $M \to N_1$, the sample labelled $(M, \underline{+}_2)$ is used, and in the reduction $N_2 \to O$, the sample labelled $(M \to N_2, \underline{+}_2)$ is used. 
Each of these samples becomes the value of the second numeral in $O$ in their respective reduction sequences, therefore in order for Church-Rosserness to be attained, they must be the same. Which elements of $L_0(M)$ must match can be described by the relation $\sim^*$:

\lo{I think there are typos above: see the corrected(?) version below.}[The term $M = \tsample + \tsample$ has the reachable skeletons $N_1 = \skeletonPlaceholder + \tsample, N_2 = \tsample + \skeletonPlaceholder, O = \skeletonPlaceholder + \skeletonPlaceholder$ and $\skeletonPlaceholder$, with reductions $M \to N_1 \to O \to \skeletonPlaceholder$ and $M \to N_2 \to O \to \skeletonPlaceholder$. 
In the reduction $M \to N_1$, the sample labelled \changed[lo]{$(M, \underline{+}_1)$} is used, and in the reduction $N_2 \to O$, the sample labelled %$(M \to N_2, \underline{+}_2)$
\changed[lo]{$(M \to N_2, \underline{+}_1)$} is used. 
Each of these samples becomes the value of the \changed[lo]{first} \lo{left?} numeral in $O$ in their respective reduction sequences, therefore in order for Church-Rosserness to be attained, they must be the same. Which elements of $L_0(M)$ must match can be described by the relation $\sim^*$:]
\end{example}
...
\lo{TODO. Use \Cref{ex:cousin sim} to give an example of $\sim_c$.}
\end{definition}
