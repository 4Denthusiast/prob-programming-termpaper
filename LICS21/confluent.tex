...
We are thus led to consider the reduction relation on skeletons (and positions in a skeleton), which can be extended from the definitions on terms in the obvious way, with $\tif{\skeletonPlaceholder < 0}{A}{B}$ reducing nondeterministically to both $A$ and $B$, and $\tsample$ reducing to $\skeletonPlaceholder$, \changed[lo]{and $(\lambda x.A) \, \skeletonPlaceholder$ reducing to $A[\skeletonPlaceholder / x]$}.
For example, we have 
\(
...
As with the traces from $I^{\mathbb N}$ used to pre-select samples to use in the standard sampling semantics, 
%there will be (with one more caveat introduced later) a sample from $I$ pre-selected for each element of $L_0(M)$, which will then be used if a $\tsample$ reduction is ever performed at that position.
\changed[lo]{modified traces, which are elements of $I^{L_0(M)}$ (with one more caveat introduced later)}, will be used to pre-select a sample from $I$ for each element of $L_0(M)$, which will then be used if a $\tsample$ reduction is ever performed at that position.
...
\end{align*}
If terms rather than skeletons were used to label samples, the set of \changed[akr]{``traces''} \lo{Readers may not realise that traces here means elements of $I^{L_0(M)}$ (but with samples labelled by terms). I propose replacing the highlighted ``traces'' by ``modified traces'' (which is introduced two paragraphs before). I think this might be a little clearer, but feel free to change back.} where $A[\tsample]$ terminates would be
\begin{align*}
...
\akr{$L_s(M)$ is the set of potential positions (quotiented by $\sim^*$) where there is a $\tsample$. I don't want to move this example until after $L_s(M)$ has actually been defined, but also I don't want to imply that $I^{L_0(M)}$ is the actual entropy space too explicitly, so I'll just leave the set $s$ is restricted to implicit for now.}
\lo{Noted.}
