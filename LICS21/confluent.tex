...
A \defn{position} is a finite sequence of steps into a term, defined inductively as
\iffalse
\begin{align*}
...
\]
The \defn{subterm of $M$ at $\alpha$}, denoted $M \mid \alpha$, is defined as
\begin{align*}
M \mid \cdot & = M \\
...
A position such that $M\mid \alpha$ does exist is said to \defn{occur} in $M$. 
%\changed[lo]{
\defn{Substitution} of $N$ at position $\alpha$ in $M$, written $M[N/\alpha]$, is defined similarly.
For example, let 
\(
...
If $\alpha_1$ is a prefix of $\alpha_2$ (written as $\alpha_1 \leq \alpha_2$), then $N_2$ is also a subterm of $N_1$. If neither $\alpha_1 \leq \alpha_2$ nor $\alpha_1 \geq \alpha_2$, the positions are said to be \defn{disjoint}. 
...
Thus we can define a more general reduction relation\footnote{In this section $\to$ always means the relation defined in Definition~\ref{def:more general red}.} $\to$.
...
\end{align*}
In each of these cases, $M \mid \alpha$ is the \defn{redex}, and the reduction \emph{takes place at $\alpha$}. Each subterm can be a redex in at most one way, but there can be multiple redexes at different positions.
\end{definition}
...
\begin{definition}[$\Rightarrow$ reduction]\rm
...
%\begin{definition}
A \defn{reduction strategy} starting from a closed term $M$ is a measurable partial function $f$ from $\mathit{Rch}(M)$ to positions, such that for any reachable term $N$ where $f$ is defined, $f(N)$ is a position of a redex in $N$, and if $f(N)$ is not defined, $N$ is a value.
...
The term \defn{terminates almost surely w.r.t.~$f$} if it terminates with $f$ for almost all $s$.
...
All of the theorems on the termination of rankable terms therefore extend to other reduction strategies too. The proofs of Theorems~\ref{thm:rankable implies termination}, \ref{thm:partial implies rankable},
...
The domain of definition of the ranking functions also needs to be changed from the reachable terms $\mathit{Rch}(M)$ to the \defn{reachable terms with respect to the reduction strategy $r$},
\[
%\begin{array}{l}
...
\begin{definition}\rm
An \defn{antitone ranking function on $M$ w.r.t.~a reduction strategy $r$} is a measurable function $f:\mathit{Rch}_r(M) \to \mathbb{R}$ such that $f(N) \geq 0$ for all $N$, and there exists an antitone function $\epsilon : \nnReal \to \pReal$ such that
\begin{itemize}
...
\end{itemize}
Any closed term for which an antitone ranking function with respect to a reduction strategy $r$ exists is called \defn{antitone rankable w.r.t.~$r$}. 
\end{definition}

\begin{definition}\rm
An \defn{antitone sparse ranking function on $M$ w.r.t.~a reduction strategy $r$} is a partial function $f : \mathit{Rch}_r(M) \rightharpoonup \mathbb R$ such that there exists some antitone function $\epsilon : \nnReal \to \pReal$ such that
\begin{inparaenum}[(i)]
...
If the antitone function is just the constant function $1$ instead, then it is called a \defn{(sparse) ranking function on $M$ w.r.t.~$r$} instead, and the term is called \defn{rankable w.r.t.~$r$}.

\begin{therm}[Confluent ranking] \label{thm:confluent ranking}
...
\medskip
If, as seems likely, Theorem~\ref{thm:antitone rankable implies termination} is complete for terms from which every reachable term is AST, then by the following theorem, Theorem~\ref{thm:confluent ranking} can't actually prove the termination of any terms not already provable by Theorem~\ref{thm:antitone rankable implies termination} and Theorem~\ref{thm:antitone partial implies rankable}.
...
%Details of the ranking function construction with and without using the confluent semantics are given in Appendix~\ref{apx:confluent}.
\end{example}
