...
Although the definition of potential positions in a term in \Cref{sec:confluent} was intended simply to define a variant of trace semantics that has a restricted version of the Church-Rosser property, it is likely to also be applicable to \changed[lo]{inference algorithms (for probabilistic programs) as well}.
...
%In the Metropolis-Hastings inference algorithm, 
\changed[lo]{In lightweight Metropolis-Hastings (LMH) or single-site MH (originally suggested by \cite{wingate2011lightweight}; see also \cite{rainforth2017Automating})},
...
\lo{Can you provide an example (and present it in the appendix)?}
