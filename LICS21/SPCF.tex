...
Following \cite{DBLP:conf/esop/CulpepperC17}, we call the probability space $(\entrosp, \Sigma_\entrosp, \mu_{\entrosp})$ the \defn{entropy space}, and elements of $\entrosp$ \defn{traces}.
...
We say that a term $M$ is \defn{almost surely terminating} (\emph{AST}) just if $\mu_{\entrosp}(\{s \mid \hbox{$M$ terminates at $s$}\}) = 1$.
