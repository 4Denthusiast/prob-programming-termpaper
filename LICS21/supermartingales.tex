...
(In subsequent applications to termination analysis, $\Omega = S$, and $\mathbb{P} = \mu_{\entrosp}$.)
...
\begin{definition}\rm
\begin{enumerate}
\item A sequence of random variables $(Y_n)_{n \geq 0}$ adapted to a filtration $(\calF_n)_{n \geq 0}$ is a \emph{supermartingale} if for all $n \geq 0$, $Y_n$ is integrable (i.e.~$\expect{|Y_n|} < \infty$), and $\expect{Y_{n+1} \mid \calF_n} \leq Y_n$ a.s.~\changed[lo]{(i.e.~for all $A \in \calF_n$, $\int_A \mathbb{P}(\dif \omega) \, Y_{n+1}(\omega) \leq \int_A \mathbb{P}(\dif \omega) \,Y_n(\omega)$).}
\item Let $\epsilon > 0$. 
...
{(For $A \in \calF$, we write ${\bf 1}_A$ for the \emph{indicator function} of $A$, i.e., the random variable defined by ${\bf 1}_A(\omega) := 1$ if $\omega \in A$, and $0$ otherwise.)} \lo{\LaTeX\ does not allow the preceding bracketed sentence to be typeset as a footnote. Strange!}
...
%\emph{Notation.}
\changed[lo]{Henceforth, fix the probability space $(S, \calF, \mu_{\entrosp})$, and a closed SPCF term $M$.
For $n \geq 0$, define the random variables $M_n(s) := \pi_0 (\red^n(M, s))$, and the filtration $(\mathcal{F}_n)_{n \geq 0}$ where $\mathcal{F}_n := \sigma(M_1, \cdots, M_n)$.}

Our first result is the following theorem.
\begin{theorem}[Rankability] 
\label{thm:rankable and strict rankable}
If a closed SPCF term $M$ is rankable (respectively, strictly rankable) by $f$ satisfying $f(N) = 0$ iff $N$ is a value, then $(f(M_n))_{n \geq 0}$ is a supermartingale (respectively, ranking supermartingale w.r.t.~stopping time $T_M: s \mapsto \min \set{n \mid f(M_n(s)) = 0}$) adapted to $(\mathcal{F}_n)_{n \geq 0}$. %where $\mathcal{F}_n = \sigma(M_1, \cdots, M_n)$.
\end{theorem}
...
Given an $\omega$-sequence (e.g.~$s \in S$) and $m \geq 0$, we write $s_{\leq m} \in I^m$ to mean the prefix of $s$ of length $m$.
%Fix a closed SPCF term $M \in \Lambda^0$, and a ranking function $f$ for it, satisfying $f(N) = 0$ iff $N$ is a value.
%Let $n \geq 0$, define the following random variables on the probability space $(S, \calF, \mu)$:
%\begin{align*}
