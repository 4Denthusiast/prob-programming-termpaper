...
\begin{definition}\rm
\begin{asparaenum}[(i)]
\item A sequence of random variables $(Y_n)_{n \geq 0}$ adapted to a filtration $(\calF_n)_{n \geq 0}$ is a \defn{supermartingale} if for all $n \geq 0$, $Y_n$ is integrable (i.e.~$\expect{|Y_n|} < \infty$), and $\expect{Y_{n+1} \mid \calF_n} \leq Y_n$ a.s.~(i.e.~for all $A \in \calF_n$, $\int_A Y_{n+1}(\omega) \, \mathbb{P}(\dif \omega) \leq \int_A Y_n(\omega) \, \mathbb{P}(\dif \omega)$).
\item Let $\epsilon > 0$. 
...
%\begin{remark}
Lemma~\ref{lem:rank-PAST} is a slight extension of \cite[Lemma 5.5]{DBLP:conf/popl/FioritiH15},
...
\quad \hbox{(Inequality~(\ref{eqn:a u ui}))}\\
&= 
...
We first state a useful property of the random variables $T_0, T_1, T_2, \dots$.
\begin{lemma}
...
\begin{enumerate}
\item Every $\transform{M}$-reduction terminates on reaching a value, or a term of the shape $E[\bot \, N_1 \cdots N_n]$.
...
\end{enumerate}
From there, all the other stopping times $T_n$ can be shown to be bounded using a similar argument by induction. There are finitely many possible values of $\transform{M_{T_n+1}}$, and for each of these possibilities, there is a finite bound on $T_{n+1}$ by the same argument as before, therefore $T_{n+1}$ is bounded by the maximum of these.

The event $\{T_n = k\}$ is equal to the event \{exactly $n$ of the terms $M_0, \dots, M_{k-1}$, and $M_k$ itself, are values or of the form $E[\tY \lambda x.N]$\}, which depends only on $M_0, \dots, M_k$ and is therefore in $\mathcal F_k$, therefore $T_n$ is a stopping time adapted to $(\mathcal F_n)_{n \geq 0}$.
\end{proof}
