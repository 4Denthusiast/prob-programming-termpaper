...
Any closed term for which a ranking (respectively, strict ranking) function exists is called \emph{rankable} (respectively, \emph{strictly rankable}). It will be demonstrated later that for any rankable term $M$, if $(M_n)_n$ is the reduction sequence starting from $M$, then $(f(M_n))_n$ is a supermartingale, and $M$ terminates almost surely, but first, some preliminaries about supermartingales.
\end{definition}
...
Using shorthand 
$\Theta_1 = \tY \, \lambda f n . \, 
\tif{n = 0}{0}{f \, (n - 1) \oplus_{1/2} f \, (n + 1)}$, 
we can define an antitone partial ranking function $f_1 : \mathit{Rch}(M_1) \rightharpoonup \nnReal$ with antitone function $\epsilon_1$ defined as follows (for $n \in \mathbb N$):
\begin{align*}
{\Theta_1} \, n 
&\mapsto 
g_1(n)
...
0 &\mapsto 0.
\end{align*}
For $n \geq 1$, $\Theta_1 \, n$ reduces in several steps to either $\Theta_1 \, (n-1)$ or $\Theta_1 \, (n+1)$, each with probability $1/2$, with one $\tY$-reduction.
\begin{align*}
  & g_1(n) - \frac{g_1(n-1) + g_1(n+1)} 2 \\
...
\end{align*}
and $\Theta_1 \, 0$ reduces to $0$ with one $\tY$-reduction with a reduction in $g_1$ of $1$, therefore by setting $\epsilon_1(x) = \frac 1 {5(e^{x-1}+1)^2}$, the condition on how much $g_1$ must decrease is met. It is also defined at $M_1 = \Theta_1 \, 10$, and is non-negative, therefore it is an antitone partial ranking function, and $M_1$ is AST.
...
\lo{The preceding sentence is unclear. Typo?} 
...
Define a \emph{skeleton} to be a term but, instead of having real constants $\underline r$, it has a placeholder $\skeletonPlaceholder$, so that each \changed[lo]{term $M$} has a skeleton $Sk(M)$, and each \changed[lo]{skeleton $S$} can be converted to a term $S[r]$ given a \changed[lo]{vector $r$} of $n$ real numbers to substitute in, where $n$ is the number of occurrences of $\skeletonPlaceholder$ in $S$. \lo{Do we actually use the notation $S[r]$ in what follows?}
...
Reduction sequences are used rather than reachable skeletons because if the same skeleton is reached twice, different samples may be needed, \lo{Give a simple example illustrating this.} but the reduction sequences of skeletons will be discussed as though they were just skeletons, identifying them with their final skeletons. With this abuse of notation, a reduction sequence $N$ (actually $N_1 \to \dots \to N_n = N$) may be said to reduce to a reduction sequence $O$, where the reduction sequence implicitly associated with the final skeleton $O$ is $N_1 \to \dots \to N_n \to O$.
...
\begin{itemize}
    ...
    \item If $M \to N$ at $\alpha$, no position in any term in $Rch(N)$ is related to $(M,\alpha)$ by $\sim_M^*$, and therefore each sample is used at most once in any reduction sequence.
