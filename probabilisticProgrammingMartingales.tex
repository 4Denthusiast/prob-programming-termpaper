...
\akr{As this proof is taking a long time to sort out, I'm just going to assume for now some of the results, and proceed from there. The results I'm assuming are that if $M \to N$, the $\sim$ relation on $Rch(N)$ is equal to the restriction of the $\sim$ relation on $Rch(M)$, restricted to $Rch(N)$.}
...
\paragraph{Reduction relation $\Rightarrow$.}
