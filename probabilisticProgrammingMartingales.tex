...
\newcommand{\tif}[3]{\textsf{if}\,(#1, #2, #3)} % space-saving concrete syntax
\newcommand{\tsample}{\textsf{sample}}
\newcommand{\tscore}{\textsf{score}}
\newcommand{\skeletonPlaceholder}{\textsf{X}} % I'm not sure what the best notation here is.
...
A position such that $M\mid P$ does exist is said to occur in $M$. 
\changed[lo]{Substitution of $N$ at position $P$ in $M$, written $M[N/P]$, is defined similarly.
For example 
\begin{align*}
...
This is mostly relevant in that in any substitution at some position, the subterms at all other positions are unaffected. \lo{The preceding sentence is unclear. Typo? Mostly $\mapsto$ most?}
...
\changed[lo]{The binary relation $\to$ is defined by the following rules, each is conditional on a redex occurring at position $P$ in the term $M$.}
\begin{align*}
...
\lo{To be clear, should write out the definition of $X \sim_c Y$.
Let the final term of the reduction sequences $X$ and $Y$ be $\hat{X}$ and $\hat{Y}$ respectively. 
Write $X \sim_c Y$ to mean for every position $P$ that occurs in $\hat X$, $(\hat X, P) \sim_c (\hat Y, P)$.
}
...
Essentially what Lemma~\ref{redexDestroyed} demonstrates is that the samples taken during any reduction sequence are independent of each other. This is made more precise in the following lemma.
...
\akr{For the standard CBV semantics, this example is indeed not a problem. 
