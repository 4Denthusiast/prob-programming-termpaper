...
Define a \emph{skeleton} to be a term but, instead of having real constants $\underline r$, it has \changed[lo]{(possibly more than one occurrence of the same)} placeholder $\skeletonPlaceholder$, so that each term has a skeleton $S = Sk(M)$ and each skeleton can be converted to a term $S[\overline r]$ given a $\overline r$ vector of $n$ real numbers to substitute in, where $n$ is the number of occurrences of $\skeletonPlaceholder$ in $S$. 
\lo{Need to resolve skeleton here with earlier definition in Sec.~\ref{sec:sampling semantics}.}
The positions in a skeleton and the reduction relation on skeletons can be extended from the definitions on terms in the obvious way, with \changed[lo]{$\tif{\skeletonPlaceholder < 0}{A_1}{A_2} \to A_i$ nondeterministically; and $M \to M[\skeletonPlaceholder / P]$, in case $M \mid P = \tsample$ and $\lambda$ does not occur after $@_2$ or $\tY$ in $P$.
...
Reduction sequences of skeletons are used rather than reachable \changed[lo]{skeletons} because if the same \changed[lo]{skeleton} is reached twice, different samples may be needed, \lo{Give a simple example illustrating this.} but they will be discussed as though they were \changed[lo]{skeletons}, by abuse of notation. 
...
The reflexive transitive closure of this relation $\sim$, written $\sim^*_M$, is used to define the set of \emph{potential positions} $L(M) = L_0(M) / \sim^*_M$, and each equivalence class can be considered as the same position as it may occur across multiple reachable \changed[lo]{skeletons}. 
If $(N,P) \sim^*_M (O,Q)$, then $N \mid P$ and $O \mid Q$ both have the same shape (i.e.~they're either \changed[lo]{both placeholders}, both variables, both applications, both $\tsample$s etc.), therefore it's well-defined to talk of the set of potential positions where there is a $\tsample$, $L_s(M)$. 
...
\changed[lo]{Since $I^{L_s(M)}$ is a countable product, the measure space is well-defined (see e.g.~\citep[Cor.~2.7.3]{AshDD00}).}
\changed[lo]{(To avoid notational clutter, we will often elide the subscript from $\sim^\ast_M$ whenever the starting skeleton $M$ of the reduction sequences is clear form the context.)}
...
If $(N_1,P_1) \sim^* (N_2,P_2)$, there are some descendants $N'_1, N'_2$ of $N_1$ resp.~$N_2$, and a position $P'$ in both of them such that $(N_1,P_1) \sim_p^* (N'_1, P') \sim_c^* (N'_2, P') \sim_p^* (N_2, P_2)$.
\end{lemma}
\begin{proof}
...
\end{align*}
\lo{$M$ and $N$ above are skeleton. So we should have $N = Sk\big(M[\underline{s(Sk(M),P)}/P]\big)$.}
...
The injection $i(M \to M_1) \circ i(M_1 \to M')$ consists of \changed[lo]{prepending} $M \to M_1 \to M'$ to each reduction sequence, but by case 4.a of $\sim$, this is equivalent to \changed[lo]{prepending} $M \to M_2 \to M'$, which is $i(M \to M_2) \circ i(M_2 \to M')$. 
...
A \emph{reduction strategy} starting from a closed skeleton $M$ is a partial function $f$ from $Rch(M)$ to positions, such that for any reachable skeleton $N$ where $f$ is defined, $f(N)$ is a position of a redex in $N$, and if $f(N)$ is not defined, $N$ is a value.
...
A closed skeleton $M$ is AST with respect to $\cbv$ iff it is AST.
\end{theorem}
\begin{proof}
...
For both $\red$ \lo{We have defined the reduction relation $\to$ on skeletons, but not (yet) $\red$.} and $\Rightarrow_{\cbv}$, the probability measure on the sample space can be used to define a measure on the reduction sequences of each finite length. 
The distributions are equal for each length, by induction, as follows.
...
For any skeleton $N$ which is not a value, there is a unique \changed[lo]{context} $E$ and redex $R$ such that $N = E[R]$. 
The position of the hole in $E$ is equal to $\cbv(N)$, so that $N \mid \cbv(N) = R$, as the cases in the definition of $\cbv$ match the cases in the definition of \changed[lo]{context}. 
If $R \neq \tsample$, let $R'$ be the result of reducing $R$ (which is equal in both versions of the semantics), then $E[R'] = N[R' / \cbv(N)]$ therefore $\red(N,s) = (E[R'],s)$ and $(N,s) \Rightarrow_{\cbv} (E[R'],s)$. 
The distribution of next skeletons, conditional on the previous skeleton, in the case that that previous skeleton reduces deterministically, is therefore equal for $\red$ and $\Rightarrow_{\cbv}$.

In the other case that $R = \tsample$, the first (skeleton) part of $\red(N,s)$ is $E[\underline{\pi_h(s)}]$. \lo{This should be $Sk(E[\underline{\pi_h(s)}])$.} 
If $k$ is the number of samples already taken in this reduction sequence, this is equivalent to $E[\underline{s_0(k)}]$, where $s_0$ is the initial sample. \lo{Should introduce a name to replace sample here.}
For any sufficiently small neighbourhood of $N$ (i.e.~containing only reduction sequences with the same skeletons), the probability of reaching this neighbourhood is independent of $s(k)$, because it depends only on the samples taken so far, and the samples are independent. 
The distribution of $s(k)$ conditional on reaching $N$ is therefore the uniform distribution on $I$. 
Similarly for the other semantics, if the distribution of remaining samples after $n$ steps is independent of the skeleton by Lemma \ref{lem:independentSamples}, therefore the distribution of $s(Sk(N),\cbv(N))$ conditional on $N$ is the uniform distribution on $I$. 
In either case, the distribution of the next skeleton conditional on the previous skeleton, in the case that it reduces randomly, is equal to the image under $r \mapsto E[\underline r]$ of the uniform distribution on $I$.

In any case, the distribution of reduction sequences after $n+1$ steps, conditional on the reduction sequence after $n$ steps, as defined by $\Rightarrow_{\cbv}$ and by $\red$, is equal; 
and by the inductive hypothesis the distributions after $n$ steps are equal, therefore by integrating over the reduction sequence up to step $n$, the distributions on reduction sequences up to step $n+1$ are equal.

The distributions on reduction sequences of any finite length as defined by $\Rightarrow_{\cbv}$ and $\red$ are therefore equal; 
therefore so is the probability of having reached a value after $n$ steps; 
therefore so is its limit, the probability of termination; 
therefore the probability of termination is 1 iff the probability of termination with respect to $\cbv$ is 1.
\end{proof}
