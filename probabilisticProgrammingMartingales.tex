...
\begin{enumerate}
    \item If $N$ reduces to $O$ with the redex at position $P$\lo{Do you mean: $M \to^\ast N \to O = N[\dotDelta / P]$ for some redex contraction $\Delta \to \dotDelta$}, and $Q$ is a position in $N$ 
    where $Q \not \geq P$, then $(N,Q) \sim_p (O,Q)$.
    %\lo{This implies that $Q$ is a position in $O$, but $O \mid Q$ is not guaranteed to exist. E.g.~$N = (\lambda x.x) 1$ and $O = 1$. Now $N \mid Q$ exists, where $Q = @_1 ; \lambda$, but $O \mid Q$ does not exist.} \lo{Ah! I overlooked the condition $Q \not\geq P = {\cdot}$.}
    ...
    \item If $N \mid P = (\tY \lambda x. A)$, $A \mid Q = C$ is a redex reducing to $C'$, $R$ exists in $C'$ and $C' \mid R \neq x$, then letting $O_1$ be $N$ reduced first at $P$ then $P;\lambda;@_1;Q$ and $O_2$ reduced at $P;\tY;\lambda;Q$ then \changed[lo]{at} $P$, $(O_1, P;\lambda;@_1;Q;R) \sim_c (O_2, P;\lambda;@_1;Q;R)$
    
    \item If $N \mid P = (\tY \lambda x. A)$, $A \mid Q = C$ is a redex reducing to $C'$, $R$ exists in $C'$ and $A \mid S = x$, then letting $O_1$ be $N$ reduced first at $P$ then $P;\lambda;@_1;S;\tY;\lambda;Q$ and $O_2$ reduced at $P;\tY;\lambda;Q$ then \changed[lo]{at} $P$, 
    \[
    (O_1, P;\lambda;@_1;S;\tY;\lambda;Q;R) \sim_c (O_2, P;\lambda;@_1;S;\tY;\lambda;Q;R)
    \]
\end{enumerate}

\begin{figure}
\includegraphics[height=0.9\textheight]{akr-diagram.pdf}
\caption{\label{fig:adk-diagram} Some examples of $\sim_p$- and $\sim_c$-rules}
\end{figure}
...
\section{Antitone-convex ranking functions}
...
M_1 = 
\big(\tY \, f \, n \, . \, 
\tif{n = 0}{0}{f \, (n - 1) \oplus_{1/2} f \, (n + 1)}\big) \, 1
\]
...
\begin{definition}
We say that a supermartingale $(Y_n)_{n \in \omega}$ adapted to filtration $(\calF_n)_{n \in \omega}$ is \emph{antitone-convex strict} if $Y_n \geq 0$, and there exists an antitone and convex measurable function $\epsilon : \nnReal \to \pReal$ satisfying
\[
\expect{Y_{n+1} \mid \calF_n} \leq Y_n - (\epsilon \circ Y_n) \cdot 1_{\set{Y_n > 0}}
\]
%We say that the supermartingale is \emph{antitone-convex strict} if $\epsilon$ is further assumed to be convex.
\end{definition}

\begin{theorem}
\label{thm:a-c strict}
Let $(Y_n)_{n \in \omega}$ be a antitone-convex strict supermartingale via function $\epsilon : \nnReal \to \pReal$, and define stopping time $T(s) := \min \set{n \mid Y_n(s) = 0}$. 
Then 
\begin{enumerate}
\item $T < \infty$ almost surely, and 
\item $\expect{T} \leq \dfrac{\expect{Y_0}}{\epsilon_0}$ where $\epsilon_0 := \epsilon(\expect{Y_0})$.
\end{enumerate}
\end{theorem}

\begin{proof}
\emph{Claim}: for all $n \in \omega$,
\(
\expect{Y_n} \leq \expect{Y_0} - \epsilon_0 \cdot \Big(\sum_{i=0}^{n-1} \mathbb{P}[Y_i > 0]\Big).
\)
...
\int_A\mu(x,\dif y)\;f(x,y)\cdot \frac{\dif \nu'}{\dif\mu}(x,y)
  \step{$f=1$ outside $\infty[\nu]$} 
\int_A\mu(x,\dif y)\;\frac{\dif \nu'}{\dif\mu}(x,y)
  \step{RN Theorem for $\sigma$-finite measures}
\int_A\nu'(x,\dif y)
  \step{definition of integral}
\nu'(x,A).
\end{calculation}
\fi
{%\everymath{displaystyle}
...
\expect{\expect{Y_{n+1} \mid \calF_n}}
  \step[\leq]{linearity of expectation}
\expect{Y_n} - \int \epsilon \circ Y_n \cdot 1_{\set{Y_n > 0}} \, \dif \mu
\; = \;
\expect{Y_n} - \int_{\set{Y_n > 0}} \epsilon \circ Y_n \, \dif \mu
  \step[\leq]{convexity of $\epsilon$, and Jensen's inequality}
\expect{Y_n} - \epsilon \big(\int_{\set{Y_n > 0}} Y_n \, \dif \mu\big)
  \step[=]{by assumption $Y_n \geq 0$}
\expect{Y_n} - \epsilon (\expect{Y_n})
  \step[\leq]{$\mathbb{P}[Y_n > 0] \leq 1$}
\expect{Y_n} - \epsilon (\expect{Y_n}) \cdot \mathbb{P}[Y_n > 0]
  \step[\leq]{$\epsilon$ is antitone, and $\epsilon_0 = \expect{Y_0} \geq \expect{Y_n}$}
\expect{Y_n} - \epsilon_0 \cdot \mathbb{P}[Y_n > 0]
  \step[\leq]{induction hypothesis}
%\expect{Y_0} - \epsilon_0 \cdot \Big(\sum_{i=0}^{n-1} \mathbb{P}[Y_i > 0]\Big) - \epsilon_0 \cdot \mathbb{P}[Y_n > 0]
%\step[\leq]{}
\expect{Y_0} - \epsilon_0 \cdot \Big(\sum_{i=0}^{n} \mathbb{P}[Y_i > 0]\Big)
\end{calculation}
}
...
\expect{\expect{Y_{n+1} \mid \calF_n}}\\
&\leq
\expect{Y_n} - \expect{\epsilon \circ Y_n} \cdot \mathbb{P}[Y_n > 0]\\
&\leq
\expect{Y_0} - \epsilon_0 \cdot \Big(\sum_{i=0}^{n-1} \mathbb{P}[Y_i > 0]\Big)
- \expect{\epsilon \circ Y_n} \cdot \mathbb{P}[Y_n > 0]\\
&\leq
\expect{Y_0} - \epsilon_0 \cdot \Big(\sum_{i=0}^{n} \mathbb{P}[Y_i > 0]\Big)
\end{align*}
\fi

It follows from the claim that $\sum_{i=0}^{\infty} \mathbb{P}[Y_i > 0]$ converges; hence $\lim_{n \to \infty} \mathbb{P}[Y_n > 0] = 0$ and so $\mathbb{P}[T < \infty] = 1$.

For 2, it suffices to prove: if $T < \infty$ a.s., then $\expect{T} = \sum^\infty_{i=1} \mathbb{P}[T \geq i]$.
\end{proof}

\paragraph{Results to prove:}
\begin{enumerate}
\item (AST Soundness) If a closed SPCF term $M$ is antitone-convex rankable, then $T^{\tY}_M < \infty$ a.s.~(equivalently $M$ is AST), and $\expect{T^{\tY}_M} < \infty$. 
...
To prove the existence of antitone-convex ranking functions, we appeal \lo{(hopefully)} to a partial ranking function theorem for antitone-convex ranking functions.
...
Note that $(g_1 (n))_{n \geq 0}$ satisfies the recurrence relation
\[
z_0 = 0, \qquad
z_n > 1/2 \cdot z_{n-1} - 1/2 \cdot z_{n+1} \quad (n \geq 1)
\]
and set the antitone and convex function $\epsilon_1$ to be 
\[
x \mapsto \big(g_1(x) - 1/2 \cdot g_1(x-1) - 1/2 \cdot g_1(x+1)\big) = \frac{1}{2^{x+1}}.
\]

For $M_3$, we use the function $g_3(n) := \frac{3^n - 1}{3^{n-1}}$, and set $\epsilon_3$ to be 
\[
x \mapsto \big(g_3(x) - 2/3 \cdot g_3(x-1) - 1/3 \cdot g_3(x+2) \big) = 
\frac{28}{3^{x+2}}.
\]

\subparagraph{Questions.} 
\begin{enumerate}
\item Can we remove the convexity assumption?
\item Complete method for AST?

%\lo{Example~\ref{ex:0 probability reachable} seems somewhat artificial: its validity depends on $I$ being the closed interval. There is no reason why we can't define $I$ to be the open interval $(0, 1)$, in which case, $Rch(M)$ would consist of all terms that are reachable from $M$ with non-zero probability.}
\end{enumerate}
