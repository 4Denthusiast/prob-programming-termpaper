...
In the other case, there is some overlap between the regions involved in $Y \sim_c X$ and $X \sim_c X_1$, but the last reduction in $X \sim_c X_1$ is later than the last reduction in $Y \sim_c X$. There are only 2 reductions in $X$ involved in $X \sim_c X_1$, therefore the region involved in $Y \sim_c X$ ends on the first of the reductions involved in $X \sim_c X_1$. Now consider the specific sequence of positions of the reductions in $X$ involved in either of these steps. Let the last of these positions be $\alpha$, the first $\gamma$, and the second-last, third-last and so on $\beta_1, \beta_2, \dots$ respectively, and let the position of the first relevant reduction in $Y$ be $\delta$. Then $X \sim_c X_1$ swaps $\beta_1$ and $\alpha$, and $\alpha$ comes before $\beta_1$ in CBV order (i.e.~$\alpha < \beta_1$ or $\alpha$ is left of $\beta_1$), and the step $Y \sim_c X$ either swaps $\delta$ and $\gamma$ resulting in $\gamma, \beta_k, \dots, \beta_1$, or swaps $\delta$ with the next (or several next) reduction (s) resulting in $\gamma, \beta_1$, depending on the direction of the $\sim_c$ step. In each of the cases below, it is required to show that the canonical sequence $(X_i)_i$ eventually reaches some term which is the same as one in $(Y_j)_j$, and from that point on, they match, therefore they reach the same end result: $X_{cbv} = Y_{cbv}$. Not all of the cases below are disjoint, but they do cover all of the remaining possibilities.
...
\begin{itemize}
    ...
    \item $\gamma, \beta_k, \dots, \beta_1$ has length 1 (so that $\gamma = \beta_1$): In this case, $Y \sim_c X$ either by case b or by case e (where the lambda has no occurrences of its variable), therefore $\gamma < \delta$ (the position of the reduction which is essentially deleted in $Y \sim_c X$). $\alpha$ is either outside or left of $\gamma$, therefore it bears the same relation to $\delta$. The sequence $(X_i)_i$ starts by swapping $\gamma$ below $\alpha$, and $(Y_j)_j$ starts by swapping $\gamma$ then $\delta$ below $\alpha$, then what happens next depends on the shape of the term at $\alpha$, or equivalently, by which case of $\sim_c$ are $\gamma$ and $\delta$ swapped below $\alpha$. If it's cases a, c or d, each of $\gamma$ and $\delta$ corresponds to one reduction in $Y_2$, and the relative position of these is the same as the relative position of $\gamma$ and $\delta$ therefore the next step of $(X_i)_i$ involves these two reductions and deletes the one equivalent to $\delta$, so $Y_3 = X_1$. If it's case b or e, in $X_2$ there are $n$ reductions corresponding to $\delta$ and $n$ reductions corresponding to $\gamma$, and they are all disjoint except that for each image of $\delta$, there's one image of $\gamma$ that it's inside, in a position that is deleted by the reduction at that image of $\gamma$. The next $\frac{n(n+1)} 2$ steps in $(X_i)_i$ are then each image of $\delta$ in turn being swapped (by case a) past the images of $\gamma$ until they reach the one they match, at which point they get deleted. In this case therefore, $Y_{n(n+1)/2+2} = X_1$. Case f, corresponding to $\tY$-reduction, is more complicated because the images of $\gamma$ and $\delta$ are not all disjoint. For each of $\gamma$ and $\delta$, there is one image of it that may contain other such positions. Let these be $\gamma_o$ and $\delta_o$. The sequence $(Y_j)_j$ then proceeds by moving all of the images of $\delta$ which are right of $\gamma_o$ to their corresponding $\gamma$-image and deleting them, then moving all the images of $\delta$ which are after $\delta_o$ but which are inside $\gamma_o$ (one at a time) past all the images of $\gamma$ left of $\gamma_o$ by case a, then either deleting them against $\gamma_o$ by case b or e as appropriate, or moving them past it by case c or d, then possibly past some of the images of $\gamma$ inside $\gamma_o$, then deleting them once they reach the matching image of $\gamma$. Then $\delta_o$ itself is moved to $\gamma_o$ then deleted, then the remaining images of $\delta$ are similarly moved and deleted. The end result of this process is that all the images of $\delta$ are deleted and all the images of $\gamma$ are unaffected, therefore for some $j$, $Y_j = X_1$.
    \item $\alpha$ is disjoint from $\gamma$ and all the $\beta_i$: In this case, $\alpha$ is left of all the $\beta_i$, and if it isn't also left of $\gamma$, then $Y_1 = X$ and we're done. Otherwise, $(X_i)_i$ proceeds by swapping each of the $\beta_i$ in turn then $\gamma$ below $\alpha$ by case a, and $(Y_j)_j$ swaps everything up to $\delta$ to below $\alpha$ by case a, then performs the same swap as in $Y \sim_c X$, but after $\alpha$ in the reduction sequence, yielding $\alpha, \gamma, \beta_k, \dots, \beta_1$, which matches $X_{k+1}$.
    \item $\alpha > \gamma$: In this case, $Y_1 = X$.
    \item \akr{There are several more cases to go, and that last case is more typical of how complex they'll be. I'd guess around 5 more, but I haven't worked them all out precisely. Also, I might find a neater way to deal with case f, or at least not have to repeat that argument, because the case where both reductions are $\tY$-reductions should include all the complexity of any other case involving case f.}
\end{itemize}
