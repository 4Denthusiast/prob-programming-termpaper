...
\lo{What are the primitive functions $f$? All measurable functions?}

To define the reduction relation, let environments \lo{They are called \emph{evaluation contexts}. Environments mean something else.} be of the form:
\begin{align*}
...
\lo{- I assume this is a CBV language (judging from the (evaluation context) clause $v \, E$). So the first rule should be
\[E[(\lambda x.s) v] \to E[s[v/x]]\]

- In a CBV language, the $\tY$-combinator should only be applied to terms of type $(A \to B) \to (A \to B)$. 
Your rule $E[\tY v] \to E[v (\tY v)]$ will diverge ($E[v (v (v \cdots ))]$).
The redex rule for the Y-combinator should be
\[
\tY (\lambda y.s) \to \lambda z.s[\tY{(\lambda y.s)}/y]\,z \quad \hbox{($z$ is fresh)}
\]

}
\subsection{Sampling semantics}
This version of the reduction relation allows $\tsample$ to reduce to any number in $[0,1]$. To more precisely specify the probabilities, an additional argument is needed to determine the outcome of random samples. Let $ I = [0,1] \subset \mathbb{R} $, and let $S = I^{\mathbb{N}}$, with the Borel $\sigma$-algebra and the measure, \changed[lo]{denoted $\mu$}, given by the limit of $1 \gets I \gets I^2 \gets \cdots$, where the maps are the projections that ignore the last element. 
The maps $\pi_h:S \to I, \; \pi_t:S \to S$ popping the first element are then measurable.
\lo{What does a measurable subset of $S$ look like? Give a ``concrete'' description.}
\lo{Clarify the measure $\mu$: give a ``formula'' for computing $\mu(A)$, for $A \in \Sigma_S$. Is $\mu$ a probability measure?}
...
The limit $\red^\infty$ can then be defined as a partial function as $\lim_{n \to \infty} \red^n(M,s)$ whenever that sequence becomes constant by reaching a normal form. \lo{Define $\red^n(M,s)$.} 
A term $M$ terminates for a sample sequence $s$ if the limit $\red^\infty(M,s)$ is defined, and it terminates almost surely if $\mu(\{s \mid M \text{ terminates at } s\}) = 1$.
\lo{Why is $\{s \mid M \text{ terminates at } s\}$ a measurable subset of $S$?}
\lo{Why is your definition of a.s.~termination reasonable? 
For example, is it the case that for all $M$, $\mu(\{s \mid M \text{ terminates at } s\}) \leq 1$?}

\lo{Important: Give examples (and non-examples) of a.s.~terminating terms (such as the one you wrote on my whiteboard).}
...
Given a probabilistic program (i.e.~a term $M$), in order to construct a supermartingale to prove its a.s.~termination, a function to assign values to each reachable program state is necessary. Define a \emph{ranking function on $M$} to be a measurable function $f:\mathit{Rch}(M) \to \mathbb{R}$ such that
\begin{itemize}
    \item $f(N) \geq 0$ for all $N$
    \item $f(E[\tY N]) \geq 1 + f(E[N(\tY N)])$ \lo{Modify this in view of the revised \tY-redex rule.}
    \item $f(E[\tsample]) \geq \int_0^1 f(E[\underline{x}]) \, \mathrm{d}x$ 

    \lo{I assume you mean $\int_{I} \mathsf{Leb} (\mathrm{d} x) \, f(E[\underline{x}])$.} 
    
    \lo{For the integral to be well-defined, we need $x \mapsto E[\underline x]$ to be a measurable function $\mathbb{R} \to \mathit{Rch}(M)$.} 

    \lo{Define the measurable space $\mathit{Rch}(M)$, and the measurable space of SPCF terms.}
