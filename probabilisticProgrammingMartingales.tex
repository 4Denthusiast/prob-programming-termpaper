...
Define a \emph{skeleton} to be a term but, instead of having real constants $\underline r$, it has placeholders $\skeletonPlaceholder$, so that each term has a skeleton and each skeleton can be converted to a term given the appropriate number of real numbers to substitute in. 
\lo{Skeleton is defined in Sec.~\ref{sec:sampling semantics}.}
The positions in a skeleton and the reduction relation on skeletons can be extended from the definitions on terms in the obvious way, with \changed[lo]{$\tif{\skeletonPlaceholder < 0}{A}{B}$} reducing nondeterministically to both $A$ and $B$, \changed[lo]{and $M \to M[\skeletonPlaceholder' / P]$ for a freshly generated placeholder $\skeletonPlaceholder'$, in case $M \mid P = \tsample$ and $\lambda$ does not occur after $@_2$ or $\tY$ in $P$.
(By abuse of notation, we use the same metavariables $M, N$, etc.~for skeletons as for terms.)}
\changed[lo]{For example, we have 
\(
(\lambda x . \tif{x < 0}{x}{\skeletonPlaceholder_1}) \, \tsample
\to
(\lambda x . \tif{x < 0}{x}{\skeletonPlaceholder_1}) \, \skeletonPlaceholder_2
\to
\tif{\skeletonPlaceholder_2 < 0}{\skeletonPlaceholder_2}{\skeletonPlaceholder_1}
\to
\skeletonPlaceholder_i
\) where $i$ is 1 or 2.}

Given a closed term $M$, let $L_0(M)$ be the set of pairs, the first element of which is a \changed[lo]{$\to$-reduction} sequence of skeletons starting at \changed[lo]{the skeleton of} $M$, and the second of which is a position in the final skeleton of the reduction sequence. 
...
The injection $L_0(N) \to L_0(M)$ is trivial to define by \changed[lo]{prepending} $M \to N$ to each path, and using Lemma~\ref{lem:sim-M-N}, this induces a corresponding injection on the quotient, $L(N) \to L(M)$. 
