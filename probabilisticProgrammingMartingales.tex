...
\expect{Y_n} - \epsilon \big(\int_{\set{Y_n > 0}} Y_n \, \dif \mu\big)
  \step[\leq]{by assumption $Y_n \geq 0$}
...
\]
and set the antitone and convex function $\epsilon_1$ to be $x \mapsto g_1(x) - 1/2 \cdot g_1(x-1) - 1/2 \cdot g_1(x+1) = \frac{1}{2^{x+1}}$.

For $M_3$, we use the function $g_3(n) := \frac{3^n - 1}{3^{n-1}}$, and set $\epsilon_3$ to be $x \mapsto g_3(x) - 2/3 \cdot g_3(x-1) - 1/3 \cdot g_3(x+2) = 
\frac{28}{3^{x+2}}$.
...
\lo{Example~\ref{ex:0 probability reachable} seems somewhat artificial: its validity depends on $I$ being the closed interval. 
There is no reason why we can't define $I$ to be the open interval $(0, 1)$, in which case, $Rch(M)$ would consist of all terms that are reachable from $M$ with non-zero probability.}
\end{enumerate}
