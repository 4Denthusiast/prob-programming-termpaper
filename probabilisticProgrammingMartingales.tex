...
\red(M,s) = \left\{
    ...
    \end{array} \right .
\end{equation}
\lo{Replace (globally) ``normal form'' by ``value''.}
...
If $M_{y_n}$ isn't already in normal form, then \changed[lo]{$M_{y_n} = E[\tY \lambda x.N]$ for some $E, N$, therefore $M_{y_n+1} = E[\lambda z. N[(\tY \lambda x. N)/x] z]$} and $f(M_{y_n+1}) \leq f(M_{y_n}) - 1$, therefore if $M_{y_n}$ isn't normal form, $\mathbb{E}[X_{n+1} \mid \mathcal{F}_{y_n+1}] \leq X_n - 1$.
