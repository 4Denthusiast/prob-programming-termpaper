...
Let $Rch(M) := \{x \in \Lambda \mid \exists (y_i) : M \to y_0 \to \dots \to y_n \to x\}$, with the $\sigma$-algebra induced as a subset of $\Lambda$. 

\begin{definition}\rm
A \emph{ranking function on $M$} is a measurable function $f:\mathit{Rch}(M) \to \mathbb{R}$ such that
\begin{itemize}
...
\lo{Take $M \in \Lambda^0$ (closed SPCF terms).
Define, for each $n \in \omega$, the random variable $M_n : S \to \Lambda^0$ by $M_n(s) := \pi_0 (\red^n(M, s))$.

Given $M \in \Lambda^0$, say that $M$ is \emph{type-1} (respectively 2, 3 and 4) if $M$ has the shape $E[\tY \lambda x. N]$ (respectively $E[\tsample]$, $E[R]$ where $R$ is any other redex, and of a value).
...
f_{i, n+1}(M)(s) :=
\begin{cases}
f(M_{n+1}(s)) & \textrm{if $M_n(s)$ is type-$i$}\\
0 & \textrm{o/w}
\end{cases}
\]
...
& \leq f(M_{n})[T_1] + f(M_{n})[T_2] + f(M_{n})[T_3]
\label{eqn:ts} \\
& = f(M_n). 
\label{eqn:partition} 
\end{align}
...
Inequation~(\ref{eqn:ts}) follows from $f_{i, n+1}(M) \leq f(M_n)[T_i]$ for each $i$. 
Here we use the notation: given function $g : A \to \Real$ and $U \subseteq A$, $g[U] : A \to \Real$ denotes the function
\[
g[U](x) :=
\begin{cases}
g(x) & \textrm{$x \in U$}\\
...
\end{cases}
\]
and write $T_i = \{s \in S \mid \textrm{$M_n(s)$ is type-$i$}\}$.
[** To show $\expect{f(M_{n+1}) \mid \mathcal{F}_n} \leq f(M_n)$ a.s., it suffices to show
\(\int_A \expect{f(M_{n+1}) \mid \mathcal{F}_n} \dif \mu \leq \int_A f(M_n) \dif \mu \), for all $A \in \mathcal{F}_n$. **]
...
% \]
Finally, \Cref{eqn:partition} holds because $\{T_1, T_2, T_3, T_4\}$ is a partition of $S$.
}
