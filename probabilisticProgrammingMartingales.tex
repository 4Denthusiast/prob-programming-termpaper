...
\changed[lo]{The binary relation $\to$ is defined by the following rules, each is conditional on a redex occuring at position $P$ in the term $M$ in question.}
\begin{align*}
...
\begin{enumerate}
    ...
    \lo{This implies that $Q$ is a position in $O$, but $O \mid Q$ is not guaranteed to exist. E.g.~$N = (\lambda x.x) 1$ and $O = 1$. Now $N \mid Q$ exists, where $Q = @_1 ; \lambda$, but $O \mid Q$ does not exist.}
    \lo{Ah! I overlooked the condition $Q \not\geq P =
    {\cdot}$.}
    

    \lo{I find this confusing. Can you express it formally?}
...
\begin{example}
See \Cref{fig:adk-diagram} for examples of $\sim_p$- and $\sim_c$-rules.
\end{example}
...
\lo{I can suggest possible \LaTeX\ packages for drawing. Can you draw them and upload a scan?}
