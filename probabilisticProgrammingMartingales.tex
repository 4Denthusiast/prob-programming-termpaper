...
\lo{NOTATION. To be consistent, we write $\lambda x. M$ rather than $\lambda x : M$.}
...
\lo{The preceding definition does not place any constraint on the stopping time $T$ (other than that it is w.r.t.~the filtration $(\calF_n)_{n \in \omega}$).
However for the definition to make sense, $T$ must be the stopping time $T(s) := \min \set{n \mid Y_n(s) = 0}$. So I propose the following.}
...
\end{theorem}
\akr{TODO: Move proof of this theorem here, and edit it so that it's actually proving this, rather than trying to prove something stronger.}
